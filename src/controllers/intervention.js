...
exports.detailsIntervention = async (req, res, next) => {
    ...
    res.json({ id: result.id, status: result.status, data: result.intervention_status, available_client: result.available_client });
}
...
exports.swapIntervention = async (req, res) => {
    ...
    const intervention = await prisma.intervention.create({
        data: {
            ...
            status: "Cloturée",
            device: { connect: { imei: req.body.imei } },
            client: { connectOrCreate: { where: { cin: client.cin }, create: client } },
        }
    });
...
exports.detailsSwap = async (req, res, next) => {
    ...
    res.json({ id: result.id, status: result.status, data: result.Swap_status });
}
